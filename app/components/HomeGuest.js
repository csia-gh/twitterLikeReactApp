  ...
  function reducer(draft, action) {
    switch (action.type) {
      ...
      case 'usernameAfterDelay':
        if (!draft.username.hasErrors && draft.username.value.length < 3) {
          draft.username.hasErrors = true
          draft.username.message = 'Username must be at leat 3 characters.'
        }
      ...
      case 'passwordAfterDelay':
        if (!draft.password.hasErrors && draft.password.value.length < 12) {
          draft.password.hasErrors = true
    ...
    if (state.submitCount) {
      ...
      async function fetchResults() {
        try {
          ...
          appDispatch({
            type: 'flashMessage',
            value: 'Congrats! Welcome to your new acount.',
          })
        } catch (error) {
          console.log('There was a problem or the request was cancelled.')
          console.log(error)
